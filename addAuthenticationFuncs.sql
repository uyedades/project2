...
users.password, users.div, users.dept from users;
create or replace rule ins_user as on insert to users_view
do instead insert into users(name,username,enc_pass,salt,div,dept)
values (new.name,new.username,
make_pass(make_salt(new.password)||new.password),
make_salt(new.password), new.div, new.dept);
